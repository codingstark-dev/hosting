...
export default async (req: NextApiRequest, res: NextApiResponse) => {
  const { domainQuery } = req.query;
  const domain = await fetch(
    'https://ambitionhost.in/user/index.php?rp=/domain/check',
    {
      headers: {
        accept: '*/*',
  ...
  const spotlight = await fetch(
    'https://ambitionhost.in/user/index.php?rp=/domain/check',
    {
      ...
      referrer:
        'https://ambitionhost.in/user/cart.php?a=add&domain=register&query=https://pinterestdownloader.com/',
      referrerPolicy: 'strict-origin-when-cross-origin',
